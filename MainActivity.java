...
import ru.project.MobileAssistant.Favorite.Favorite_Adapter;
import ru.project.MobileAssistant.Favorite.Favorite_List;
...
public class MainActivity extends AppCompatActivity implements MessageListener {
    ...
    int int_columnID_main, int_columnID_phone, int_columnID_phone_name, int_columnID_ruble, int_columnID_comment, int_columnID_date, int_columnID_card, int_row_position, int_columnID_favorite;
    int int_columnID_favorite_main,int_columnID_delete;
...
//    ListView lv;
    ...
    //    public static final String switchOnOff_text = "switchOnOff_text";
...
//        toolbar.setBackgroundResource(R.drawable.selector_delete);
//        Objects.requireNonNull(getSupportActionBar()).setDisplayHomeAsUpEnabled(true);
//        Objects.requireNonNull(toolbar_main.getNavigationIcon()).setColorFilter(getResources().getColor(R.color.cardview_shadow_start_color), PorterDuff.Mode.SRC_ATOP);
...
//        Lisе<city> output_transfer = (ListView) findViewById(R.id.output_transfer);
...
//        output_transfer_arrayList = new ArrayList<String>(Arrays.asList("111,222,333,444,555,666".split(",")));
...
//        output_transfer_adapter = new ArrayAdapter<>(this, R.layout.my_simple_list_item_multiple_choice, output_transfer_arrayList);
//        output_phone_adapter = new ArrayAdapter<>(this, R.layout.my_simple_list_item_multiple_choice, output_phone_arrayList);
//        output_balance_adapter = new ArrayAdapter<>(this, R.layout.my_simple_list_item_multiple_choice, output_balance_arrayList);
//        output_history_adapter = new ArrayAdapter<>(this, R.layout.my_simple_list_item_multiple_choice, output_history_arrayList);
//        output_thank_adapter = new ArrayAdapter<>(this, R.layout.my_simple_list_item_multiple_choice, output_thank_arrayList);
...
//        output_transfer.liste
        ...
        loadData_card();
        //loadData_output_transfer();
        //loadData_output_phone();
        //loadData_output_balance();
        //loadData_output_history();
        //loadData_output_thank();
        loadData_transfer();
        loadData_phone();
        loadData_balance();
        loadData_history();
        ...
        load_switch_vibrate();
        closeKeyboard();
//        Log.d("My_log","MainActivity onCreat");
            ...
            public void afterTextChanged(Editable s) {
                if (text_transfer.getText().toString().isEmpty()) {
                    ...
                    if ((ContextCompat.checkSelfPermission(MainActivity.this, android.Manifest.permission.READ_CONTACTS) == PackageManager.PERMISSION_GRANTED) && (arraylist_contact.size() > 0)) {
                        ...
                        //String text_01 = text.replaceAll("[^\\d.]", "");
...
//                        constraintSet.clear(R.id.ic_rubl,ConstraintSet.END);
                    constraintSet.applyTo(constraintLayout);
                } else {
                    if (!text_transfer.getText().toString().isEmpty()) {
                        send.setEnabled(true);
                    }
//                        if (money_transfer.getText().toString().length()>1) {
                    constraintSet.clone(constraintLayout);
                    TransitionManager.beginDelayedTransition(constraintLayout);
...
//                    if (!toolbar.getTitle().toString().equals(getString(R.string.Label))) {change_color_list_on_transparent();}
                closeKeyboard();
//                if (action_mode != null) {
//                    action_mode.finish();
//                } //отключаем выносное меню удаления и детализации
...
//--------------------------------------------------------------------------------------------
        //следим за долгим нажатием на пункт листа
        output_transfer.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE_MODAL);
        output_transfer.setMultiChoiceModeListener(new AbsListView.MultiChoiceModeListener() {
            @Override
            public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
                // TODO Auto-generated method stub
                int checkedCount = output_transfer.getCheckedItemCount();
                //меняем меню в зависимости от количества выбранных пунктов
                if (checkedCount == 1) {
                    MenuItem item = menu.findItem(R.id.template);
...
//                mode.finish();
            ...
            public boolean onCreateActionMode(ActionMode mode, Menu menu) {
                mode.getMenuInflater().inflate(R.menu.menu_template, menu);
                action_mode = mode;
                return true;
            }
            ...
            public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.delete:
                        ...
                        sparseBooleanArray_checked = output_transfer.getCheckedItemPositions(); //получаем позицию выделенного пункта в листе, через boolean
                        for (int i = itemCount - 1; i >= 0; i--) { //данный метод реализцется если ты хочешь выполнить удаление, т.к. при удалении одно пункта список сдвигается вверх на один пункт
//                        for (int i = 0; i < itemCount; i++) {
                            if (sparseBooleanArray_checked.get(i)) {
//                                Log.d("My_log", "List: " + output_transfer_arrayList.get(i));
...
//                                            Log.d("My_log", "DB: " + cursor_dataDB.getString(int_columnID_main));
        ...
        output_phone.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE_MODAL);
        output_phone.setMultiChoiceModeListener(new AbsListView.MultiChoiceModeListener() {
            @Override
            public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
                // TODO Auto-generated method stub
                int checkedCount = output_phone.getCheckedItemCount();
                //меняем меню в зависимости от количества выбранных пунктов
                if (checkedCount == 1) {
                    MenuItem item = menu.findItem(R.id.template);
                    item.setVisible(true);
            ...
            @Override
            public void onDestroyActionMode(ActionMode mode) {
//                output_transfer_adapter.removeSelection();
            }
            ...
            public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.delete:
                        ...
                        sparseBooleanArray_checked = output_phone.getCheckedItemPositions(); //получаем позицию выделенного пункта в листе, через boolean
                        for (int i = itemCount - 1; i >= 0; i--) { //данный метод реализцется если ты хочешь выполнить удаление, т.к. при удалении одно пункта список сдвигается вверх на один пункт
//                        for (int i = 0; i < itemCount; i++) {
                            if (sparseBooleanArray_checked.get(i)) {
                                check_equals_while = true;
...
//                        Log.d("My_log", "" + itemCount);
        ...
        output_balance.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE_MODAL);
        output_balance.setMultiChoiceModeListener(new AbsListView.MultiChoiceModeListener() {
            @Override
            public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
                // TODO Auto-generated method stub
                int checkedCount = output_balance.getCheckedItemCount();
                //меняем меню в зависимости от количества выбранных пунктов
                if (checkedCount == 1) {
                    MenuItem item = menu.findItem(R.id.template);
                    item.setVisible(true);
            ...
            public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.delete:
                        ...
                        sparseBooleanArray_checked = output_balance.getCheckedItemPositions(); //получаем позицию выделенного пункта в листе, через boolean
                        for (int i = itemCount - 1; i >= 0; i--) { //данный метод реализцется если ты хочешь выполнить удаление, т.к. при удалении одно пункта список сдвигается вверх на один пункт
//                        for (int i = 0; i < itemCount; i++) {
                            if (sparseBooleanArray_checked.get(i)) {
                                check_equals_while = true;
...
//                        Log.d("My_log", "" + itemCount);
        ...
        output_history.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE_MODAL);
        output_history.setMultiChoiceModeListener(new AbsListView.MultiChoiceModeListener() {
            @Override
            public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
                // TODO Auto-generated method stub
                int checkedCount = output_history.getCheckedItemCount();
                //меняем меню в зависимости от количества выбранных пунктов
                if (checkedCount == 1) {
                    MenuItem item = menu.findItem(R.id.template);
                    item.setVisible(true);
            ...
            public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.delete:
                        ...
                        sparseBooleanArray_checked = output_history.getCheckedItemPositions(); //получаем позицию выделенного пункта в листе, через boolean
                        for (int i = itemCount - 1; i >= 0; i--) { //данный метод реализцется если ты хочешь выполнить удаление, т.к. при удалении одно пункта список сдвигается вверх на один пункт
//                        for (int i = 0; i < itemCount; i++) {
                            if (sparseBooleanArray_checked.get(i)) {
                                check_equals_while = true;
        ...
        output_thank.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE_MODAL);
        output_thank.setMultiChoiceModeListener(new AbsListView.MultiChoiceModeListener() {
            @Override
            public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
                // TODO Auto-generated method stub
                int checkedCount = output_thank.getCheckedItemCount();
                //меняем меню в зависимости от количества выбранных пунктов
                if (checkedCount == 1) {
                    MenuItem item = menu.findItem(R.id.template);
                    item.setVisible(true);
            ...
            public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.delete:
                        ...
                        sparseBooleanArray_checked = output_thank.getCheckedItemPositions(); //получаем позицию выделенного пункта в листе, через boolean
                        for (int i = itemCount - 1; i >= 0; i--) { //данный метод реализцется если ты хочешь выполнить удаление, т.к. при удалении одно пункта список сдвигается вверх на один пункт
//                        for (int i = 0; i < itemCount; i++) {
                            if (sparseBooleanArray_checked.get(i)) {
                                check_equals_while = true;
...
//                        Log.d("My_log", "" + itemCount);
    ...
    //изменяем цвет пунктов списка с серого на transparent
    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public void change_color_list_on_transparent() {
        if (dropdown.getSelectedItem().equals("Перевод ")) {
            toolbar.getNavigationIcon().setColorFilter(getResources().getColor(R.color.colorGreen), PorterDuff.Mode.SRC_ATOP);
            //при нажатии на стрелку, очищаем menu_contexual- toolbar_main и публикуем main-toolbar_main
            toolbar.getMenu().clear();
            toolbar.inflateMenu(R.menu.main);
            getSupportActionBar().setTitle(R.string.Label);
            Objects.requireNonNull(getSupportActionBar()).setDisplayHomeAsUpEnabled(false);
            int itemCount = output_transfer_adapter.getCount();
            SparseBooleanArray checkedItemPositions = output_transfer.getCheckedItemPositions();
            if (checkedItemPositions != null) {
                for (int i = itemCount - 1; i >= 0; i--) {
//                                if (checkedItemPositions.get(i)) {
                    try {
                        output_transfer.getChildAt(i).setBackgroundColor(Color.TRANSPARENT);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
//                                }
            ...
            getSupportActionBar().setTitle(R.string.Label);
            Objects.requireNonNull(getSupportActionBar()).setDisplayHomeAsUpEnabled(false);
            int itemCount = output_phone_adapter.getCount();
            SparseBooleanArray checkedItemPositions = output_phone.getCheckedItemPositions();
            if (checkedItemPositions != null) {
                for (int i = itemCount - 1; i >= 0; i--) {
                    try {
                        output_phone.getChildAt(i).setBackgroundColor(Color.TRANSPARENT);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            getSupportActionBar().setTitle(R.string.Label);
            Objects.requireNonNull(getSupportActionBar()).setDisplayHomeAsUpEnabled(false);
            int itemCount = output_balance_adapter.getCount();
            SparseBooleanArray checkedItemPositions = output_balance.getCheckedItemPositions();
            if (checkedItemPositions != null) {
                for (int i = itemCount - 1; i >= 0; i--) {
                    try {
                        output_balance.getChildAt(i).setBackgroundColor(Color.TRANSPARENT);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            getSupportActionBar().setTitle(R.string.Label);
            Objects.requireNonNull(getSupportActionBar()).setDisplayHomeAsUpEnabled(false);
            int itemCount = output_history_adapter.getCount();
            SparseBooleanArray checkedItemPositions = output_history.getCheckedItemPositions();
            if (checkedItemPositions != null) {
                for (int i = itemCount - 1; i >= 0; i--) {
                    try {
                        output_history.getChildAt(i).setBackgroundColor(Color.TRANSPARENT);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            getSupportActionBar().setTitle(R.string.Label);
            Objects.requireNonNull(getSupportActionBar()).setDisplayHomeAsUpEnabled(false);
            int itemCount = output_thank_adapter.getCount();
            SparseBooleanArray checkedItemPositions = output_thank.getCheckedItemPositions();
            if (checkedItemPositions != null) {
                for (int i = itemCount - 1; i >= 0; i--) {
                    try {
                        output_thank.getChildAt(i).setBackgroundColor(Color.TRANSPARENT);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
...
//                                            Log.d("My_log", "DB: " + cursor_dataDB.getString(int_columnID_main));
                                    ...
                                    output_transfer_adapter.notifyDataSetChanged();
                                    Log.d("My_log","menu delete" + sparseBooleanArray_checked);
                                }
                            })
                            .setNegativeButton(R.string.no, new DialogInterface.OnClickListener() {
                                @Override
...
//                                        try{
//                                            check_color =true;
//                                            listViewColor = (ColorDrawable) output_phone.getChildAt(i).getBackground();
//                                            Log.d ("My_log",""+listViewColor.getColor()); //регистрация перемнной, без нее не будет работать блок
//                                        }catch (Exception e){ e.printStackTrace(); check_color =false;}
//                                        try{
//                                            colorId = listViewColor.getColor();
//                                            Log.d ("My_log",""+colorId); //регистрация перемнной, без нее не будет работать блок
//                                        }catch (Exception e){ e.printStackTrace(); }
//                                        if (Color.LTGRAY == colorId && check_color) {
//                                            check_equals_while = true;
//                                            cursor_dataDB = InputDB.getListContents_phone();
//                                            int_columnID_main = cursor_dataDB.getColumnIndex(InputContract.TableEntry.COL_PHONE);
//                                            if(cursor_dataDB.getCount() == 0) {
...
//                                        try{
//                                            check_color =true;
//                                            listViewColor = (ColorDrawable) output_balance.getChildAt(i).getBackground();
//                                            Log.e ("Mylog","listViewColor - "+listViewColor); //регистрация перемнной, без нее не будет работать блок
//                                        }catch (Exception e){ e.printStackTrace(); check_color =false;Log.d ("My_log","error");}
//                                        try{
//                                            colorId = listViewColor.getColor();
//                                            Log.e ("Mylog","colorId - "+colorId); //регистрация перемнной, без нее не будет работать блок
//                                        }catch (Exception e){ e.printStackTrace(); check_color =false;Log.d ("My_log","error1");}
//                                        Log.d ("My_log","for - "+i);
////                                    if (Color.LTGRAY == colorId && check_color) {
//                                        if (check_color) {
//                                            check_equals_while = true;
...
//                                            if(cursor_dataDB.getCount() == 0) {
//                                            }else {
//                                                Log.d ("My_log","before while");
//                                                while (cursor_dataDB.moveToNext() && check_equals_while) {
//                                                    Log.d ("My_log","afte while");
//                                                    if(output_balance_arrayList.get(i).equals(cursor_dataDB.getString(int_columnID_main))) {
//                                                        Log.d ("My_log","came");
//                                                        Log.d ("My_log",""+i);
//                                                        Log.d ("My_log",""+cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)));
//                                                        check_equals_while = false;
//                                                        output_balance.getChildAt(i).setBackgroundColor(Color.TRANSPARENT);
//                                                        output_balance_adapter.remove(output_balance_arrayList.get(i));
//                                                        InputDB.delete_Data_balance(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)));
...
//                                        try{
//                                            check_color =true;
//                                            listViewColor = (ColorDrawable) output_history.getChildAt(i).getBackground();
//                                            Log.d ("My_log",""+listViewColor); //регистрация перемнной, без нее не будет работать блок
//                                        }catch (Exception e){ e.printStackTrace(); check_color =false;}
//                                        try{
//                                            colorId = listViewColor.getColor();
//                                            Log.d ("My_log",""+colorId); //регистрация перемнной, без нее не будет работать блок
...
//                                        try{
//                                            check_color =true;
//                                            listViewColor = (ColorDrawable) output_thank.getChildAt(i).getBackground();
//                                            Log.d ("My_log",""+listViewColor); //регистрация перемнной, без нее не будет работать блок
//                                        }catch (Exception e){ e.printStackTrace(); check_color =false;}
//                                        try{
//                                            colorId = listViewColor.getColor();
...
//            get_contact_List();
    ...
    public void onRequestPermissions_sms() {
        if (!ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.SEND_SMS) ||
            ...
            dialog_permission.setMessage("У вас нет разрешения для работы с SMS, данный доступ необходим для отправки/получения запросов в Сбербанк. Получить разрешение?")
                    .setCancelable(false)
                    .setPositiveButton(R.string.yes, new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
            ...
            dialog_permission.setMessage("У вас нет разрешения для работы с SMS, данный доступ необходим для отправки/получения запросов в Сбербанк. " +
...
//                    Log.d("My_log", "test12" + string_phoneNumber);
...
//            String dateToStr = format.format(date_today);
            if (dropdown.getSelectedItem().equals("Перевод ")) {
                date_today = new Date();
...
//                Pattern p_transfer02 = Pattern.compile("перевод .* Баланс:.*р$");
//                Matcher m_transfer02 = p_transfer02.matcher(message);
...
//                alert.getWindow().setType(WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
                    ...
                    string_message_convert = "Перевод для " + mes_name[0] + " выполнен." + "\n" + message;
                        ...
                        if (cursor_dataDB.getCount() == 0) {
                            ...
                            while (cursor_dataDB.moveToNext() && check_equals_while) {
                                if (string_message_convert.equals(cursor_dataDB.getString(int_columnID_main))) {
                                    ...
                                    InputDB.delete_Data_transfer(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)));
                                    InputDB.addData_transfer(string_message_convert, text_transfer.getText().toString(), text_transfer_contact.getText().toString(), money_transfer.getText().toString(), text_transfer_comment.getText().toString(), date_today.toString(), "0", "0"); //добавляем в базу
                                }
                            }
                            cursor_dataDB.close();
                        }
                        ...
                        if (cursor_dataDB.getCount() == 0) {
                            ...
                            while (cursor_dataDB.moveToNext() && check_equals_while) {
                                if (message.equals(cursor_dataDB.getString(int_columnID_main))) {
                                    ...
                                    InputDB.delete_Data_transfer(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)));
                                    InputDB.addData_transfer(message, text_transfer.getText().toString(), text_transfer_contact.getText().toString(), money_transfer.getText().toString(), text_transfer_comment.getText().toString(), date_today.toString(), "0", "0"); //добавляем в базу
                                }
                            }
                            cursor_dataDB.close();
                        }
...
//                Pattern p_phone02 = Pattern.compile("Оплата .* Баланс:.*р$");
//                Matcher m_phone02 = p_phone02.matcher(message);
                        ...
                        if (cursor_dataDB.getCount() == 0) {
                            ...
                            while (cursor_dataDB.moveToNext() && check_equals_while) {
                                if (string_message_convert.equals(cursor_dataDB.getString(int_columnID_main))) {
                                    ...
                                    InputDB.delete_Data_phone(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)));
                                    InputDB.addData_phone(string_message_convert, text_phone.getText().toString(), text_phone_contact.getText().toString(), money_phone.getText().toString(), date_today.toString(), "0", "0"); //добавляем в базу
                                }
                            }
                            cursor_dataDB.close();
                        }
...
//                    alert.setTitle("Уведомление");
                    alert.show();
                    /*
                    Toast toast = Toast.makeText(MainActivity.this, "Оплата телефона для " + mes_number[0] + " выполнена." + "\n" + message, Toast.LENGTH_LONG);
                    toast.setGravity(Gravity.CENTER_HORIZONTAL, 0, 0);
                    toast.show();
                     */
                } else if (check_put_button_send && message.matches("(?is)" + ".*" + Pattern.quote("Получен повторный запрос") + ".*") || message.matches("(?is)" + ".*" + Pattern.quote("Операция не выполнена") + ".*")) {
                        ...
                        if (cursor_dataDB.getCount() == 0) {
                            ...
                            while (cursor_dataDB.moveToNext() && check_equals_while) {
                                if (message.equals(cursor_dataDB.getString(int_columnID_main))) {
                                    ...
                                    InputDB.delete_Data_phone(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)));
                                    InputDB.addData_phone(message, text_phone.getText().toString(), text_phone_contact.getText().toString(), money_phone.getText().toString(), date_today.toString(), "0", "0"); //добавляем в базу
                                }
                            }
                            cursor_dataDB.close();
                        }
...
//                Pattern p_get_card = Pattern.compile("Баланс по карте");
                Matcher m_get_card = p_get_card.matcher(message);
//                Pattern p_card = Pattern.compile("^Список подключенных карт:");
//                Matcher m_card = p_card.matcher(message);
                if (m_get_card.find()) {
                        ...
                        if (cursor_dataDB.getCount() == 0) {
                            ...
                            while (cursor_dataDB.moveToNext() && check_equals_while) {
                                if (message.equals(cursor_dataDB.getString(int_columnID_main))) {
                                    ...
                                    InputDB.delete_Data_balance(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)));
                                    InputDB.addData_balance(message, text_balance.getText().toString(), date_today.toString(), "0", "0"); //добавляем в базу
                                }
                            }
                            cursor_dataDB.close();
                        }
                ...
                date_today = new Date();
                Pattern p_get_card = Pattern.compile("карты, подключенные к мобильному банку");
//                Pattern p_get_card = Pattern.compile("выписка по карте");
                Matcher m_get_card = p_get_card.matcher(message);
//                Pattern p_card = Pattern.compile("^Список подключенных карт:");
//                Matcher m_card = p_card.matcher(message);
                if (m_get_card.find()) {
                        ...
                        if (cursor_dataDB.getCount() == 0) {
                            ...
                            while (cursor_dataDB.moveToNext() && check_equals_while) {
                                if (message.equals(cursor_dataDB.getString(int_columnID_main))) {
                                    ...
                                    InputDB.delete_Data_history(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)));
                                    InputDB.addData_history(message, text_history.getText().toString(), date_today.toString(), "0", "0"); //добавляем в базу
                                }
                            }
                            cursor_dataDB.close();
                        }
...
//                    alert.setTitle("Уведомление");
                ...
                date_today = new Date();
                Pattern p_get_card = Pattern.compile("карты, подключенные к мобильному банку");
//                Pattern p_get_card = Pattern.compile("спасибо");
                Matcher m_get_card = p_get_card.matcher(message);
//                Pattern p_card = Pattern.compile("^Список подключенных карт:");
//                Matcher m_card = p_card.matcher(message);
                if (m_get_card.find()) {
                        ...
                        if (cursor_dataDB.getCount() == 0) {
                            ...
                            while (cursor_dataDB.moveToNext() && check_equals_while) {
                                if (message.equals(cursor_dataDB.getString(int_columnID_main))) {
                                    ...
                                    InputDB.delete_Data_transfer(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)));
                                    InputDB.addData_thank(message, text_thank.getText().toString(), date_today.toString(), "0", "0"); //добавляем в базу
                                }
                            }
                            cursor_dataDB.close();
                        }
...
//            TypedArray a = context.obtainStyledAttributes(R.style.AppTheme, new int[] {R.attr.selectableItemBackground});
...
//            view.setBackgroundResource(attributeResourceId);
    ...
    public class DeleteAdapter extends ArrayAdapter implements OnCreateContextMenuListener {
        ...
        public View getCustomView(final int position, View view, ViewGroup parent) {
            ...
            viewholder_adapter_delete.txtTitle = (TextView) view.findViewById(R.id.text_adapter_dalete);
            ...
            if (dropdown.getSelectedItem().equals("Перевод ")) {
                ...
                viewholder_adapter_delete.txtTitle.setOnClickListener(new View.OnClickListener() {
                    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
                    @Override
                    public void onClick(View view) {
                        cursor_dataDB = InputDB.getListContents_transfer();
...
//Затем добавьте следующий метод в MainActivity для обновления пользовательского интерфейса
    private void LoadData_output_Cash() {
        ...
        if (cursor_dataDB.getCount() == 0) {
            ...
            int idx = cursor_dataDB.getColumnIndex(InputContract.TableEntry.COL_TRANSFER);
            while (cursor_dataDB.moveToNext()) {
                InputDB.update_Data_transfer_delete(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)), "0"); //записываем значение в базу
        ...
        if (cursor_dataDB.getCount() == 0) {
            ...
            int idx = cursor_dataDB.getColumnIndex(InputContract.TableEntry.COL_PHONE);
            while (cursor_dataDB.moveToNext()) {
                InputDB.update_Data_phone_delete(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)), "0"); //записываем значение в базу
        ...
        if (cursor_dataDB.getCount() == 0) {
            ...
            int idx = cursor_dataDB.getColumnIndex(InputContract.TableEntry.COL_BALANCE);
            while (cursor_dataDB.moveToNext()) {
                InputDB.update_Data_balance_delete(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)), "0"); //записываем значение в базу
        ...
        if (cursor_dataDB.getCount() == 0) {
            ...
            int idx = cursor_dataDB.getColumnIndex(InputContract.TableEntry.COL_HISTORY);
            while (cursor_dataDB.moveToNext()) {
                InputDB.update_Data_history_delete(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)), "0"); //записываем значение в базу
        ...
        if (cursor_dataDB.getCount() == 0) {
            ...
            int idx = cursor_dataDB.getColumnIndex(InputContract.TableEntry.COL_THANK);
            while (cursor_dataDB.moveToNext()) {
                InputDB.update_Data_thank_delete(cursor_dataDB.getInt(cursor_dataDB.getColumnIndex(InputContract.TableEntry._ID)), "0"); //записываем значение в базу
